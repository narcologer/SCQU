...
- or /	Toggle the priority of tile under cursor (or selection if existant) between high and low
...
Ctrl+C	Copy Selection if existant
Ctrl+X	Cut Selection if existant
Ctrl+V	Paste copied selection to the position pointed to by the current cursor

Del	Clear tile under cursor, or selection rectangle if existant
...
- When scrolling vertically off screen using Home/End, some leftover graphics data might still be displayed at the bottom of the screen.
